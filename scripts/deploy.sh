...
# 배포 시작한 날짜와 시간을 기록
echo "EXIST_BLUE: 결과  $(sudo docker-compose -p ${DOCKER_APP_NAME}-blue -f docker-compose-blue.yml ps)" >> /opt/deploy.log
...
#  BLUE_HEALTH=$(sudo docker-compose -p ${DOCKER_APP_NAME}-blue -f docker-compose-blue.yml ps | grep Up)
  BLUE_HEALTH=$(sudo docker-compose -p ${DOCKER_APP_NAME}-blue -f docker-compose-blue.yml ps)
  ...
  if [ -z "$BLUE_HEALTH" ]; then
    ...
    sudo docker exec -it frontend-blue tar -czvf /frontend/dist/archive.tar.gz -C /frontend/dist .
    sudo docker cp frontend-blue:/frontend/dist/archive.tar.gz /usr/share/nginx/html
...
# blue가 실행중이면 green up
  ...
  sleep 100
  echo "GREEN_HEALTH: 결과  $(sudo docker-compose -p ${DOCKER_APP_NAME}-green -f docker-compose-green.yml ps)" >> /opt/deploy.log
  GREEN_HEALTH=$(sudo docker-compose -p ${DOCKER_APP_NAME}-green -f docker-compose-green.yml ps)
